...
function renderBacktestChart(candles, trades) {
  ...
  const markers = [];
  trades.forEach(t => {
    markers.push({ time: t.entry_date, position: 'belowBar', color: '#4C8DFF', shape: 'arrowUp', text: 'Entrada' });
    const exitColor = t.outcome === 'objetivo' ? '#2FD98A' : t.outcome === 'stop' ? '#FF5C72' : '#8B93A7';
    markers.push({ time: t.exit_date, position: 'aboveBar', color: exitColor, shape: 'arrowDown', text: t.outcome });
  });
...
document.getElementById('backtest-run-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    if (!data.num_trades) {
      ...
      renderBacktestChart(data.candles || [], []);
    ...
    renderBacktestChart(data.candles, data.trades);
    document.getElementById('backtest-content').classList.remove('hidden');
  } catch (err) {
